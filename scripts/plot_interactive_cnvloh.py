...
from plot_interactive_cnvloh_bokeh import generate_genome_plot as cnvloh_bokeh_plot_genome

#### FUNCTIONS ####

### Accessory functions -- these are things that might get used more than once for various CNV-LOH processing purposes ####

def order_genome_info(genome_info_dict:dict, mains_only:bool=True, size_cutoff:int=None):
    autosomes = {}
    allosomes = []
    other = []
...
def process_genes_toplot(genes_table):
    ...
    for i in range(0,len(list(loh_nearby_genes))):
        if True in [list(loh_nearby_genes)[i],list(loh_genes)[i],list(all_gain_genes)[i],list(all_loss_genes)[i]]: # ,list(all_focal_genes)[i]]:
...
def segment_to_bins(segment_length:int, bin_size:int=(10 ** 6), segment_start:int=0):
    bin_starts = []
    bin_ends = []
    bin_start = 0
...
def find_point_bin(point_pos:int, num_bins:int, segment_start:int, segment_end:int, bin_size:int=1000000):
    segment_len = segment_end - segment_start
    bin_index = max(min(num_bins,math.ceil((((point_pos - segment_start - bin_size)/segment_len)*(num_bins+1)))),1)-1
    return bin_index

## Adjusts LOH calls when the GATK results do not reflect reality
def add_segment_avg_baf(segment:dict, min_len:int=1000000):
    print(f"{segment['position']['chrom']}:{segment['position']['start']}-{segment['position']['end']}")
...
def downweight_excessive_vars(segment:dict, highest_decile:float = 20.0, low_cutoff:float=-0.10):
    ...
    if len(variants) == 0:
        ...
        for i in variants:
            distance = min(abs(prev_variant['position'] - i['position']),abs(segment['position']['end']-i['position']))
            #print(distance)
...
def loh_rescue(segments_dict:dict, metadata_dict:dict=None, min_len:int=5000000):
    ...
    vars_per_mb = sorted([x['loh']['vars_per_mb'] for x in segments_dict if x['loh']['vars_per_mb'] is not None])
    ...
    print(f'Median variants per megabase: {median_vars_per_mb}\nLowest decile variants per megabase: {lowest_decile}')
    ...
    if abs(lowest_decile - median_vars_per_mb) >= (median_vars_per_mb * 0.33):
        fill_homoz = True
        print("Lowest decile of variants/megabase deviates greatly from median. Filling in homozygotes.")
    else:
        fill_homoz = False
    corrections = []
    for i in range(0,len(segments_dict)):
        ...
        allele_bafs, allele_weights = downweight_excessive_vars(segments_dict[i], highest_decile)
        weighted_sum_baf = 0
        ...
        adj_vars_per_mb = total_weight / (segments_dict[i]['position']['length']/(10 ** 6))
        print(f"Variants per mb: {segments_dict[i]['loh']['vars_per_mb']} | Adjusted: {adj_vars_per_mb}")
        
        if segments_dict[i]['loh']['vars_per_mb'] is None:
            print(i)
            print(f"\tSegment {label} has no variants but was too short to fill ({segments_dict[i]['position']['length']}).")
            
        elif (segments_dict[i]['loh']['vars_per_mb'] < lowest_decile or adj_vars_per_mb < lowest_decile) and (segments_dict[i]['position']['length'] > min_len) and fill_homoz is True:
            print(label)
            #segments_dict[i]['loh']['adj_baf'] = (segments_dict[i]['loh']['baf_from_vars']*segments_dict[i]['loh']['vars_per_mb']) / lowest_decile
            
            segments_dict[i]['loh']['adj_baf'] = (weighted_sum_baf / (lowest_decile * (segments_dict[i]['position']['length'] / (10 ** 6))))
            
        elif str(segments_dict[i]['loh']['loh_allele_fraction']).lower() == "nan" or segments_dict[i]['loh']['loh_allele_fraction'] is np.nan or (segments_dict[i]['loh']['loh_allele_fraction'] - (weighted_sum_baf / total_weight)) > 0.1: #Add an abs() to the second part if you want to be able to reduce LOH also.
            print(label)
...
def cnvloh_json2dfs(cnvloh_dict: str, chromosome:str=None, coverage_bin_size:int=1000000):
    ...
    for chromosome in chroms_list:
        for i in range(0,len(cnvloh_dict)):
            if cnvloh_dict[i]['position']['chrom'] == chromosome:
                ...
                calc_seg_len = cnvloh_dict[i]['position']['end'] - cnvloh_dict[i]['position']['start']
                segment_bin_starts, segment_bin_ends = segment_to_bins(calc_seg_len, coverage_bin_size, cnvloh_dict[i]['position']['start'])
                calc_num_bins = len(segment_bin_starts)
                segment_num_bins.append(calc_num_bins)
                segment_points = []
                segment_reads = 0
                calc_bin = 0
                for j in range(0,len(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'])):
                    calc_midpoint = round((cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['start']+cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['end'])/2)
                    calc_sub_len = cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['end']-cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['start']
                    calc_bin = find_point_bin(calc_midpoint, calc_num_bins, cnvloh_dict[i]['position']['start'], cnvloh_dict[i]['position']['end'],coverage_bin_size)
                    subseg_parent_index.append(seg_index)
                    subseg_chrom.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['chrom'])
                    subseg_x0.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['start'])
                    subseg_x1.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['end'])
                    subseg_mid.append(calc_midpoint)
                    subseg_len.append(calc_sub_len)
                    subseg_half_len.append((cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['end']-cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['start'])/2)
                    subseg_y.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['log2_copy_ratio'])
                    subseg_reads.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['reads'])
                    if cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['reads']:
                        segment_reads += cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['reads']
                    segment_points.append(cnvloh_dict[i]['cnv']['cnv_supporting_subsegments'][j]['log2_copy_ratio'])
                    subseg_bin.append(calc_bin)
            
                if 'loh' in cnvloh_dict[i]:
                    loh_supporting_points = cnvloh_dict[i]['loh']['loh_supporting_points']
                    ...
                    elif loh_supporting_points > 0 and 'loh_allele_fraction' in cnvloh_dict[i]['loh']:
                    ...
                    if loh_supporting_points > 0:
                        for j in range(0,len(cnvloh_dict[i]['loh']['loh_supporting_variants'])):
                            var_chrom.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['chrom'])
                            var_x.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['position'])
                            var_y.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['alt_vaf'])
                            ref_reads.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['ref_count'])
                            ref_base.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['ref_allele'])
                            alt_reads.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['alt_count'])
                            alt_base.append(cnvloh_dict[i]['loh']['loh_supporting_variants'][j]['alt_allele'])
                else:
                    pass
    
    seg_df = pd.DataFrame(list(zip(segment_chrom,segment_index,segment_id,segment_pos_start,segment_pos_end,segment_cnv,segment_cnv_marks,segment_cnv_reads, segment_cnv_stdev,segment_loh,segment_loh_marks,segment_loh_reads,segment_num_bins)),columns=['CHROM','INDEX','ID','START','END','LOG2','LOG2_MARKS','LOG2_READS','LOG2_STDEV','LOH','LOH_MARKS','LOH_READS','NUM_BINS'])
    seg_df = seg_df.sort_values('INDEX').reset_index(drop=True)
    cnv_df = pd.DataFrame(list(zip(subseg_chrom, subseg_parent_index, subseg_x0,subseg_x1,subseg_mid, subseg_len, subseg_half_len, subseg_reads, subseg_y, subseg_bin)),columns=['CHROM','PARENT_INDEX','START','END','MIDPOINT','LEN','HALF_LEN','READS','LOG2','BIN'])
    cnv_df = cnv_df.sort_values('PARENT_INDEX').reset_index(drop=True)
    try:
        loh_df = pd.DataFrame(list(zip(var_chrom,var_x,var_y,ref_base,alt_base,ref_reads,alt_reads)),columns=['CHROM','POS','VAF','REF','ALT','REF_READS','ALT_READS'])
    except:
        loh_df = None
...
def calc_coverage_bins(seg_df, cnv_df, bin_width:int=1000000, min_support:int=2, abs_positions:bool=False):
    bins_dict={}
    all_chroms = []
    all_segs = []
    all_bins = []
    ...
    for i in set(seg_df['CHROM']):
        ...
        for j in set(chrom_df['INDEX']):
            ...
            bins_dict[i][j]={}
            segment_bin_starts, segment_bin_ends = segment_to_bins(seg_end - seg_start, bin_width, seg_start)
            ...
            for k in bins:
                ...
                for l in range(0,len(log2s)):
                    ...
                    if k not in bins_dict[i][j]:
                        bins_dict[i][j][k] = {'start':int(segment_bin_starts[int(k)]),'end':int(segment_bin_ends[int(k)]), 'log2_scores':[point_log2],'reads':point_reads}
                    else:
                ...
                bins_dict[i][j][k]['mean']=sum(bins_dict[i][j][k]['log2_scores'])/len(bins_dict[i][j][k]['log2_scores'])
                bins_dict[i][j][k]['stdev']=np.nan_to_num(np.std(bins_dict[i][j][k]['log2_scores']))
    ...
    cnv_bins_df = pd.DataFrame(list(zip(all_chroms,all_segs,all_bins,all_starts,all_ends,all_points,all_reads,all_means,all_stdevs)),columns=['CHROM','SEG','BIN','START','END','POINTS','READS','MEAN','STDEV'])
    return cnv_bins_df
...
def setup_cnvloh_plot(genome_info,chromosome, include_helper=True):
    ...
    if include_helper:
        ...
        plot_rows = 2
    fig = make_subplots(rows=plot_rows, cols=1,shared_xaxes=True,vertical_spacing=0.005)
    ...
    fig.add_vrect(x0=centromere_start,x1=centromere_end,fillcolor="Grey", opacity=0.5,
        layer="below", line_width=0, row=1,col=1)
    boundary_df = pd.DataFrame(list(zip([0,chromosome_length],[-1,1])),columns=['X','Y'])
    fig.add_trace(go.Scatter(x=list(boundary_df['X']),y=list(boundary_df['Y']),mode="markers",opacity=0),row=1,col=1)
    fig.add_trace(go.Scatter(x=[0,chromosome_length],y=[0,0],fill="toself",mode="lines",name="Zero Point"),row=1,col=1)
    ...
    if include_helper:
        ...
        boundary_df = pd.DataFrame(list(zip([0,chromosome_length],[-1,1])),columns=['X','Y'])
        fig.add_trace(go.Scatter(x=list(boundary_df['X']),y=list(boundary_df['Y']),mode="markers",opacity=0),row=2,col=1)
        
    fig.add_vrect(x0=centromere_start,x1=centromere_end,fillcolor="Grey", opacity=0.5,
        layer="below", line_width=0,row=plot_rows,col=1)
    boundary_df = pd.DataFrame(list(zip([0,chromosome_length],[0,1])),columns=['X','Y'])
    fig.add_trace(go.Scatter(x=list(boundary_df['X']),y=list(boundary_df['Y']),mode="markers",opacity=0),row=plot_rows,col=1)
        
    fig.update_layout(height=400*plot_rows, width=1000,title_text=f'CNV-LOH: {chromosome}')
...
def fig_add_cnv_plot(fig, seg_df, cnv_df,plot_row=1):
    ...
    fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines+markers",name="CNV Supporting Marks"),row=plot_row,col=1)
    fig['data'][-1]['line']['color']='#329efb'
    
    plot_x,plot_y = cnv_to_segs(seg_df)
    fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="CNV Called Segments"),row=plot_row,col=1)
...
def fig_add_helper_plot(fig, genes_to_plot_dict, plot_row=2):    
    # Plot Gains #
    # Strong
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_gain_genes_df'][genes_to_plot_dict['strong_gain_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Gain"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#329efb'
    # Weak
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_gain_genes_df'][genes_to_plot_dict['weak_gain_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Gain (Weak)"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#164c90'
    # Plot Losses #
    # Strong
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_loss_genes_df'][genes_to_plot_dict['strong_loss_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Loss"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#f63729'
    # Weak
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_loss_genes_df'][genes_to_plot_dict['weak_loss_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Loss (Weak)"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#ab261d'
    # Plot Focal Events #
    # Gain
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_focal_genes_df'][genes_to_plot_dict['focal_gain_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Gain (Focal)"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#0431f9'
    # Loss
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['all_focal_genes_df'][genes_to_plot_dict['focal_loss_genes']],to=1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="Copy Loss (Focal)"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#f62400'
    # Plot LOH #
    # LOH
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['genes_table'][genes_to_plot_dict['loh_genes']],to=-1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="LOH"),row=plot_row,col=1)
        fig['data'][-1]['line']['color']='#fbc300'
    # Nearby LOH
    plot_x,plot_y = genes_to_rects(genes_to_plot_dict['genes_table'][genes_to_plot_dict['loh_nearby_genes']],to=-1)
    if len(plot_x) > 0:
        fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="LOH (Nearby)"),row=plot_row,col=1)
    ...
    if not genes_to_plot_dict['plotted_genes_df'].empty:
        fig.add_trace(go.Scatter(
            ...
            y=[math.sin(i)*0.9 for i in range(0,len(list(genes_to_plot_dict['plotted_genes_df']['POSITION'])))],
            mode="markers+text",
            name="Affected Genes",
            text=list(genes_to_plot_dict['plotted_genes_df']['SYMBOL']),
            textposition="top center"),row=plot_row,col=1)
...
def fig_add_loh_plot(fig,seg_df,loh_df,plot_row=3):
    ...
    fig.add_trace(go.Scatter(x=plot_x,y=plot_y,fill="toself",mode="lines",name="LOH Regions"),row=plot_row,col=1)
    fig['data'][-1]['line']['color']='#fbc300'
    ...
    fig.add_trace(go.Scatter(
        ...
        y=list(loh_df['VAF']),
        mode="markers",
        name="Supporting Variants"),row=plot_row,col=1)
...
def plot_chrom_html_bokeh(chromosome, genome_info, cnvloh_dict, out_path, genes_table=None, genes_list=None, focal_events=None, output_type:str="json", cnvloh_dict_2:dict=None, focals_dict:dict=None, metadata:dict=None):
    ...
    try:
        cnvloh_bokeh_plot(genes_table=genes_table_chrom, genes_list=genes_list, cnvloh_dna_dfs=df_dict, chrom_info_dict=genome_info[chromosome], chromosome=chromosome, outpath=plot_path, output_type=output_type, cnvloh_dna_dfs_2=df_dict_2, focals_dict=focals_dict, male_adjustment=is_male)
    except Exception as e:
        print('Exception: ' + str(e))
        traceback.print_exc()
    return None

def plot_genome_html_bokeh(genome_info, cnvloh_dict, out_path, genes_table=None, genes_list=None, focal_events=None, output_type:str="json", cnvloh_dict_2:dict=None, focals_dict:dict=None, metadata:dict=None):
    ...
    cnvloh_bokeh_plot_genome(genes_table=genes_table, cnvloh_dna_dfs=df_dict, genome_info=genome_info, genes_list=genes_list, outpath=plot_path, output_type=output_type, cnvloh_dna_dfs_2=df_dict_2, focals_dict=focals_dict, male_adjustment=is_male)
    
    return None

## Go through and plot all the chromosomes in a genome info file ##
def plot_all_chrom_html(genome_info, cnvloh_dict, out_path, genes_table=None, genes_list=None, output_type:str="json", cnvloh_dict_2:dict=None, focals_dict:dict=None, metadata:dict=None):
    #genes_table = subset_genes_table(genes_table,genes_list)
    time_start = time.time()
    plot_genome_html_bokeh(out_path=out_path, genome_info=genome_info, cnvloh_dict=cnvloh_dict, genes_table=genes_table, genes_list=genes_list, output_type=output_type, cnvloh_dict_2=cnvloh_dict_2, focals_dict=focals_dict, metadata=metadata)
    ...
    for i in genome_info:
        ...
        if (genes_table) is not None:
            ...
            genes_table_chrom = None
        plot_chrom_html_bokeh(chromosome=i, out_path=out_path, genome_info=genome_info, cnvloh_dict=cnvloh_dict, genes_table=genes_table_chrom, genes_list=genes_list, output_type=output_type, cnvloh_dict_2=cnvloh_dict_2, focals_dict=focals_dict, metadata=metadata)
        time_elapsed = time.time() - time_start
...
def run_cnvplot_argparser():
    parser = argparse.ArgumentParser(description="Creates interactive plots for CNV-LOH.")
    parser.add_argument('--sample-name', help="(Optional) Name of CNV-LOH sample being processed.", default='cnvloh_sample')
    parser.add_argument('--chromosome', help="(Optional) Chromosome to plot. Or, enter 'genome' for genome-wide plot. If not specified, plots all chromosomes.")
    parser.add_argument('--genes-table', help="(Optional) Path to genes table file, for annotation plotting.")
    parser.add_argument('--gene-sublist', help="Path to text file with subset of genes to display. If not given, will plot all genes.")
    parser.add_argument('--genome-info', help='Path to file showing chromosome lengths and centromere positions.', required=True)
    parser.add_argument('--cnvloh-dir', help='Directory to output CNV-LOH plots.',required=True)
    parser.add_argument('--cnvloh-json', help='Path to CNV-LOH JSON. If not specified, must instead use all three of Counts, Hets, and Modelsegs.')
    parser.add_argument('--focals-json', help='(Optional) Path to JSON containing gene and exon focal CNV events.', default=None)
    parser.add_argument('--hets-file', help="Path to CNV-LOH *.hets.tsv File")
    parser.add_argument('--counts-file', help="Path to CNV-LOH *.counts.tsv.denoisedCR.tsv")
    parser.add_argument('--modelsegs-file', help="Path to CNV-LOH *.modelFinal.seg file")
    ...
    parser.add_argument('--out-format', help='(Optional) Output format.', default="html", choices=["html","json"])
    parser.add_argument('--cnvloh-json-2', help="Path to CNV-LOH JSON, for optional comparator sample.",default=None)
    return parser
...
if __name__ == '__main__':
    ...
    if not cnvloh_json:
        ...
        if not modeled_seg_file or not cnv_subseg_file or not loh_calls_file:
            raise Exception('Insufficient data provided to begin plotting. Must specify either a CNV-LOH JSON, or all three of ModeledSeg, Counts, and Hets.')
        else:
            ...
            cnvloh_json = os.path.join(output_dir, f'cnvloh_output_{data_type}-{software}.json')
            generate_cnvloh_json(output_dir, sample_name, modeled_seg_file, cnv_subseg_file, loh_calls_file, data_type, case_name, cnv_params, software, software_version, sample_type)
        ...
        if os.path.exists(genes_table_file):
            ...
            if gene_sublist_file:
                if os.path.exists(gene_sublist_file):
                    try:
                        gene_sublist = list(pd.read_csv(gene_sublist_file,header=None)[0].astype(str))
                    except:
                        print('Gene list is an empty file. Disabling gene plotting.')
                        gene_sublist = []
                else:
                    raise Exception('A gene list file was specified but the file does not exist.')
            else:
                gene_sublist = None
        else:
    ...
    if cnvloh_json_2:
        with open(cnvloh_json_2, 'r') as cnvloh_json_file_2:
            ...
            segments_dict_2, metadata_dict_2 = loh_rescue(cnvloh_dict_2, metadata_dict_2)
    else:
    ...
    if chromosome:
        if chromosome.lower() == "genome":
            plot_genome_html_bokeh(out_path=interactive_plots_dir, genome_info=genome_info, cnvloh_dict=segments_dict, genes_table=genes_table, genes_list=gene_sublist, output_type=out_format, cnvloh_dict_2=segments_dict_2, focals_dict=focals_dict, metadata=metadata_dict)
        else:
            plot_chrom_html_bokeh(chromosome=chromosome, genome_info=genome_info, cnvloh_dict=segments_dict, out_path=interactive_plots_dir, genes_table=genes_table, genes_list=gene_sublist, output_type=out_format, cnvloh_dict_2=segments_dict_2, focals_dict=focals_dict, metadata=metadata_dict)
    else:
        plot_all_chrom_html(genome_info=genome_info, cnvloh_dict=segments_dict, out_path=interactive_plots_dir, genes_list=gene_sublist, genes_table=genes_table, output_type=out_format, cnvloh_dict_2=segments_dict_2, focals_dict=focals_dict, metadata=metadata_dict)
